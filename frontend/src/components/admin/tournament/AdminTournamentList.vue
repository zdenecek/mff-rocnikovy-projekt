<template>
    ...
    <v-row v-if="showFilter">
        ...
        <v-col lg="3"> 
            <v-combobox density="compact" class="category-filter" hide-details="auto" v-model="filter.type" :items="TournamentTypes" item-value="value" :return-object="false"
    ...
    <v-row>
        <v-col class="flex spaced">
            <v-btn variant="tonal" prepend-icon="mdi-plus-circle-outline" :to="{name: 'admin-add-tournament'}">Přidat turnaj</v-btn>
    ...
    <v-row>
        <v-col>
            <v-data-table :headers="headers" :items="tournamentsFiltered" item-key="id" :loading="loading" v-model="selected" show-select>
                ...
                <template v-slot:item.actions="{ item }">
                    <v-btn :to="{name:'admin-edit-tournament', params: { id: item.id}}"> <v-icon>mdi-pencil</v-icon></v-btn>
                    <v-btn color="red" @click="deleteTournament(item)"> <v-icon>mdi-delete</v-icon></v-btn>
                </template>
...
import { TournamentType, TournamentTypes } from '@/model/Tournament';
...
const selectedTournaments = computed(() => selected.value.map(id => tournaments.value.find(t => t.id === id)))
